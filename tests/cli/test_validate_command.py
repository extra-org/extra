...
def test_validate_passes_on_enterprise_knowledge_assistant_demo() -> None:
    ...
    orchestration, a local MCP, local tools, shared/agent-scoped resolvers, a
    protected agent, and all five hook lifecycle points. Nothing else in the
    test suite parses it, so it can silently drift out of sync with the schema.
    This is intentionally the same offline check `agentctl validate` runs: no
    LLM calls, no MCP network, no tool execution — hooks are
    imported/instantiated but their methods are never invoked.
    """
