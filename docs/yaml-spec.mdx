...
Pick whichever matches how your MCP server actually reads the tag — check its docs or ask whoever built it. `type` must be exactly `header` or `query_param`; anything else fails validation with a clear error.
