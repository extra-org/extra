...
`agents.yml` is the complete definition of your agent system. It declares every agent, what each one can do, and how requests are routed between them. Extra reads this file, validates it, and runs a live system from it — nothing else is required.

A spec has two parts:

- **Declarations** — the building blocks: agents, orchestrators, tools, MCP servers, resolvers, hooks.
- **`graph`** — how those blocks connect at runtime. Indentation is the routing topology.
