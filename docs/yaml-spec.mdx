...
`agents.yml` is the complete definition of your agent system. It declares every agent, what each one can do, and how requests are routed between them. Extra reads this file, validates it, and compiles it into a running system.

The YAML alone isn't the whole system — you still write the prompt files it references, and implement any tools or resolvers you declare. `agentctl generate` scaffolds all of that for you from the spec, so you fill in logic, not boilerplate.
...
| [`graph`](#graph) | Yes | How agents and orchestrators connect — the routing topology |
| [`orchestrators`](#orchestrators) | No | Routers that decide which agent handles a request |
| [`agents`](#agents) | No | Executors that run an LLM and can call tools |
...
Every example on this page builds toward the same system: a customer support bot for an online store. It routes incoming messages to the right department, and each department has its own tools and data.
...
An agent is an **executor**. Once the router sends it a request, it runs an LLM with its own prompt and can call whatever tools or MCP servers it's been given access to.
...
`{{current_date}}` and `{{customer_name}}` are filled in automatically before the agent runs — see [resolvers](#resolvers) below.

---

## `tools`

A tool is a Python function the agent can choose to call. You describe what it does; Extra generates the stub; you write the logic.
...
`agentctl generate` creates one stub file per tool. You fill in the real implementation:
...
Give every agent access by adding the tool id to its `tools` list — see [`agents`](#agents) above.
...
By default, tags are sent as an `X-MCP-Tool-Tag` header. If your server expects something else:
...
Implementation, generated by `agentctl generate`:
...
This runs before every outgoing MCP request and attaches an `Authorization` header read from the environment — the token never touches a prompt, a log line, or the YAML file itself.
