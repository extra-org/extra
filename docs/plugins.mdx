...
Run `agentctl generate --config agents.yml` to create all stubs from your YAML.
...
`agentctl generate` produces the file layout. Fill in the methods:
...
```bash
agentctl generate agents.yml               # default: all stubs
...
If you run `agentctl` from different directories, declare `import_roots` so Python can always find your package:
